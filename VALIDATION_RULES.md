...
**Prose content quality.** `validate` checks that content exists (rule 6), not whether it's any good, whether a `business_rule`'s Rationale is well-written or whether a `feature`'s Acceptance Criteria are actually testable. That's a human/agent judgment call, not a mechanical one.
