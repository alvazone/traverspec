...
6. Before presenting the result, check `traverspec/graph.yaml` (or `traverspec list` if the CLI is available, per the rule in `start_here.md`) to confirm you haven't created a duplicate of something that already existed before this ingestion started, and run `traverspec validate` if it's available (per `start_here.md`) — fix anything it reports before moving on.
