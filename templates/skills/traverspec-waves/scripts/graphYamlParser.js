...
exports.GraphYamlParseError = void 0;
exports.parseGraphYamlText = parseGraphYamlText;
/**
 ...
 */
class GraphYamlParseError extends Error {
    constructor(message, lineNumber) {
        super(lineNumber !== undefined ? `graph.yaml line ${lineNumber}: ${message}` : `graph.yaml: ${message}`);
        this.name = 'GraphYamlParseError';
    }
}
exports.GraphYamlParseError = GraphYamlParseError;
const TOP_LEVEL_KEYS = ['epics', 'nodes', 'edges'];
const UNSUPPORTED_VALUE_PREFIXES = ['{', '[', '|', '>', '&', '*', '!', "'", '"'];
const VALIDATE_POINTER = 'Then run `traverspec validate` to confirm the fix.';
function stripUnsupportedMarkers(text, lineNumber) {
    const trimmed = text.trim();
    if (trimmed === '---' || trimmed === '...') {
        throw new GraphYamlParseError(`this line is a YAML document marker ('${trimmed}'), which this lightweight parser doesn't support. ` +
            `Fix: delete this line — graph.yaml must be a single document with no document markers. ${VALIDATE_POINTER}`, lineNumber);
    }
}
function assertSupportedValue(value, lineNumber) {
    if (value === '')
        return;
    const first = value[0];
    if (UNSUPPORTED_VALUE_PREFIXES.includes(first)) {
        throw new GraphYamlParseError(`this value starts with '${first}', which is YAML quoting/flow-style/block-scalar/anchor syntax this ` +
            `lightweight parser can't read. Fix: rewrite it as a plain unquoted scalar, matching the format used ` +
            `elsewhere in graph.yaml (e.g. 'path: assets/feature/x.md', not a quoted or bracketed value). ${VALIDATE_POINTER}`, lineNumber);
    }
}
/**
 ...
 */
function parseGraphYamlText(text) {
    const lines = text.split('\n').map((l) => (l.endsWith('\r') ? l.slice(0, -1) : l));
    const sections = { epics: [], nodes: [], edges: [] };
    let i = 0;
    while (i < lines.length) {
        ...
        if (!rawLine.match(/^\S/) || !topMatch) {
            throw new GraphYamlParseError(`this line doesn't start with a recognized top-level key (${TOP_LEVEL_KEYS.join('/')}: ...), found: "${rawLine}". Fix: rewrite this line so it starts one of those three sections, or delete ` +
                `it if it's stray content that doesn't belong in graph.yaml. ${VALIDATE_POINTER}`, lineNumber);
        }
        const key = topMatch[1];
        const rest = topMatch[2].trim();
        if (!TOP_LEVEL_KEYS.includes(key)) {
            throw new GraphYamlParseError(`'${key}:' isn't a section graph.yaml supports — only ${TOP_LEVEL_KEYS.join(', ')} are valid. ` +
                `Fix: rename this section to one of those, or delete it. ${VALIDATE_POINTER}`, lineNumber);
        }
        ...
        if (rest !== '') {
            throw new GraphYamlParseError(`'${key}:' should be followed by nothing (a list of items on the following lines) or '[]' for an empty ` +
                `list, but has trailing content: "${rest}". Fix: move that content onto its own indented '- key: value' ` +
                `line below, or delete it. ${VALIDATE_POINTER}`, lineNumber);
        }
        ...
        while (i < lines.length) {
            ...
            if (!dashMatch) {
                ...
                if (line.match(/^\S/))
                    break; // new top-level key, let the outer loop handle it
                throw new GraphYamlParseError(`expected a list item ('- key: value') or a new top-level key here, found: "${line}". Fix: correct this ` +
                    `line's formatting to match a list item (e.g. '  - id: ...'), or delete it. ${VALIDATE_POINTER}`, i + 1);
            }
            const indent = dashMatch[1].length;
            if (itemIndent === null)
                itemIndent = indent;
            if (indent !== itemIndent) {
                throw new GraphYamlParseError(`this list item's '-' is indented ${indent} spaces, but earlier items in '${sectionKey}' use ${itemIndent}. ` +
                    `Fix: change this line's indentation to ${itemIndent} spaces to match the rest of the section. ${VALIDATE_POINTER}`, i + 1);
            }
            const item = { fields: {}, lineNumber: i + 1 };
            ...
            while (i < lines.length) {
                ...
                if (fLine.match(/^(\s*)-\s+[a-zA-Z_]+:/))
                    break; // next item
                if (fLine.match(/^\S/))
                    break; // next top-level key
                const fieldMatch = fLine.match(/^(\s+)([a-zA-Z_]+):\s?(.*)$/);
                if (!fieldMatch) {
                    throw new GraphYamlParseError(`expected a 'key: value' field line within a list item here, found: "${fLine}". Fix: correct this ` +
                        `line's formatting to a plain 'key: value' field. ${VALIDATE_POINTER}`, i + 1);
                }
                const fIndent = fieldMatch[1].length;
                if (fieldIndent === null)
                    fieldIndent = fIndent;
                if (fIndent !== fieldIndent) {
                    throw new GraphYamlParseError(`this field is indented ${fIndent} spaces, but earlier fields in '${sectionKey}' use ${fieldIndent}. ` +
                        `Fix: change this line's indentation to ${fieldIndent} spaces to match the rest of the section. ${VALIDATE_POINTER}`, i + 1);
                }
                if (fIndent <= itemIndent)
                    break; // shallower than the item itself — treat as next construct
...
function requireField(item, key, context) {
    const value = item.fields[key];
    if (value === undefined || value === '') {
        throw new GraphYamlParseError(`this ${context} is missing the required field '${key}'. Fix: add '${key}: <value>' to this entry. ${VALIDATE_POINTER}`, item.lineNumber);
    }
    return value;
}
function toEpic(item) {
    return {
...
function toNode(item) {
    const node = {
        ...
        type: requireField(item, 'type', 'node entry'),
        path: requireField(item, 'path', 'node entry'),
    };
    if (item.fields.epic)
        node.epic = item.fields.epic;
    return node;
}
function toEdge(item) {
    return {
        from: requireField(item, 'from', 'edge entry'),
        type: requireField(item, 'type', 'edge entry'),
