...
This file is about what things *are*, not how to walk between them.
...
- **`traverspec/assets/*.md`** — one file per node, containing only its human-readable content. These files never declare relationships themselves — no frontmatter, no edge lists. If you're looking for how something connects to something else, that answer is in `graph.yaml`, not the `.md` file.

Treat these two as a pair. The `.md` file tells you what a thing is; `graph.yaml` tells you what it connects to. You typically need both.
...
| `decision` | A record of an intentional deviation from what would otherwise look like the correct or consistent pattern. | Always paired with an `overrides` edge — see Section 3. If a node's content seems inconsistent with a rule or pattern elsewhere, check for a `decision` node overriding it before assuming the spec is wrong or stale. |
...
If a project has never used `ui_component` and you're not sure whether to introduce it, don't — ask, or default to describing UI behavior as prose inside the relevant `feature` file. More generally, if content doesn't map cleanly onto any of these node types at all, stop and ask the person rather than forcing it into the closest option — a forced fit now is a harder problem to untangle later than a short pause to clarify.

### 2a. Node granularity is a judgment call, not a fixed rule

There is no fixed size for a `feature`. The test: **would anything ever need to fetch this piece independently of its surrounding content?** If yes, it's probably its own node. If no, it's a section of prose inside a larger node.

Example: an entire multi-stage AI pipeline (upload → transcribe → diarize → summarize) can correctly be a single `feature` node, with the internal stages as prose sections inside that one file — *if* nothing else in the graph ever needs to point at "just the diarization step" on its own. Compare `api_contract`, which is always its own node regardless of size, because endpoints are frequently reused or referenced independently of any one feature.

When unsure whether to split a node further, ask whether a future edge would ever need to point at the smaller piece specifically. If you can't think of one, don't split it.
...
`overrides` is the one edge type with special traversal behavior — always checked, in both directions, regardless of entry point.

**`dispatches` vs `depends_on` vs `triggers`.** Easy to conflate — pick based on what kind of fact is actually being stated:
...
- `triggers` — synchronous, same-operation invocation: the `from` node's normal execution path directly calls or executes the `to` node (typically feature → api_contract).
- `dispatches` — asynchronous, completion-based causation: finishing the `from` node causes the `to` node to run afterward, out of band (a job, an event, a deferred process).

Only write a `dispatches` edge when the source node's own content explicitly states the handoff (e.g. "on completion, enqueue X"). Don't infer it from two features merely occurring near each other in a workflow.

When authoring a new edge, pick the type that matches the actual relationship, not the closest available one. If none of these describe the relationship, stop and ask rather than force-fitting an existing type — that's a signal the schema may need to grow, not a signal to improvise.

---

## 3a. When a shared `data_model` implies a feature-to-feature dependency

Two features can each have a `mutates`, `reads`, or `foreign_key`-chained relationship to the same `data_model` without that meaning anything about the order they need to exist in. Sharing a model does not, by itself, imply order.
...
- **Field-mediated prerequisite** — a feature's behavior depends on a specific field or value that a different feature's own spec explicitly states it writes (e.g. "sole writer of X"), even when the two features never mention each other by name.

Evidence for either can be split across the two nodes' own content — one file states the sole-writer fact, the other states the dependency on that value — as long as each half is itself explicit, not inferred. Connecting two independently explicit facts is not a guess, and the resulting edge is still `depends_on`, written the same as any other.

**When it isn't real.** Several features can touch the same model as independent, parallel operations — CRUD peers on one resource (four auth endpoints all touching the same session-token table, say) usually have no real order between them. Before writing an edge, check whether the feature can still function correctly *without* the other one having happened yet — a feature that touches a shared entity only inside a conditional branch ("if a record of this type already exists, do Y") is describing an optional integration point, not a hard prerequisite. Only write the edge for prerequisites a feature cannot produce meaningful, correct behavior without.

If you're not sure whether what you read counts as explicit, it doesn't — leave it unconnected rather than guess.
...
There is no `epic-to-epic` edge and no `epic-to-feature` edge. Epics are used for grouping and filtering — "show me everything under billing" — not for describing how things relate to each other. If two features in different epics are actually connected, that connection is expressed as a normal edge between the features (or between whatever they both touch) — never by relating the epics themselves.
...
- `path` — relative path to the node's `.md` content file, relative to the `traverspec/` folder itself, not the repo root — e.g. `assets/feature/checkout.md`, not `traverspec/assets/feature/checkout.md`.
- `epic` — present only on nodes that belong to one (see Section 4). Not present on `epic` entries themselves.

There is no `description` field on epics, and no `status` field on nodes. Both were considered and deliberately excluded — don't add them without checking with the person first; they were rejected for drift risk from hand-maintained fields going stale.
...
Every asset file opens with a single `# Title` heading matching the node's name. Below that, use the sections below as a **strong default, not a hard requirement**. If content doesn't cleanly fit a section, use judgment — write good prose over forcing a bad fit into a heading. Don't refuse to create a node because it's missing one of these sections.
...
The single `Constraints` column holds required/nullable, foreign key target, uniqueness, and defaults together (e.g. `FK → User, not null`). If a field's constraint is a foreign key, also add a `foreign_key` edge in `graph.yaml` — the table cell is for human readability, the edge is what makes it traversable.
...
`Code` is the application-level error code (e.g. `INVALID_REFRESH_TOKEN`); `Status` is the HTTP status. Keep them separate — multiple error codes can share one HTTP status, and error handling needs to branch on the code, not just the status.
